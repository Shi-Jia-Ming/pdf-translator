    ...
    <WorkDirectoryContext.Provider value={{workspace, setWorkspace}}>
      <div className={"size-full flex flex-row"}>
        <div className={"h-full w-11 flex flex-col align-middle justify-center"} id={"sidebar"}>
          ...
          <div
            className={"w-full h-except-10 border-r border-[#e0e0e0] transition-color duration-100 delay-100"}
            style={{backgroundColor: isLeftPanelCollapsed ? '#ffffff' : '#f6f6f6'}}>
          </div>
        </div>
        <div className={"h-full w-except-11"} id={"app-main"}>
          <PanelGroup direction="horizontal" className={"size-full"} autoSaveId={"persistence"}>
            <Panel collapsible defaultSize={leftPanelSize}
              ...
              <div className={"h-10 w-full border-t border-[#e0e0e0] cursor-pointer bg-[#fcfcfc]"}>
                <RepoSelection/>
              </div>
            </Panel>
            <PanelResizeHandle
            ...
            <Panel defaultSize={80}>
              <div
                className={"h-10 w-full border-r border-b border-[#e0e0e0] cursor-pointer bg-[#fcfcfc]"}
